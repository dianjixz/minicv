...
void imlib_draw_utf8_string(image_t *img, int x_off, int y_off, void *str, int c, float scale, int x_spacing, int y_spacing, bool mono_space) {
  const uint8_t font_len = (font_config.width / 8) * font_config.high;
  const uint8_t *string = str;
  int len = strlen(str);
  ...
  for(int i = 0, pos = 0, bytes = 0; i < len; i += bytes, pos += 1) {
    ...
    {
        case FileIn:
            file_seek(font_config.this, offset * font_len, 0);
            fread(buffer, 1, font_len, font_config.this);
            break;
        case ArrayIn:
...
{
    // 检查字库文件是否有效
    if (font_config.source == FileIn && font_config.this == NULL)
    {
        font_init(8, 12, ASCII, BuildIn, ascii);
    }
