...
/************ File OP ************/
file_obj* file_write_open(const char *path)
{
    return fopen(path, "wb");
}

file_obj* file_write_open_raise( const char *path)
{

    return  fopen(path, "wb");
}

file_obj* file_read_open(const char *path)
{

    return fopen(path, "rb");
}

file_obj* file_read_open_raise(const char *path)
{
...
file_obj* file_seek(file_obj *fp, int offset, uint8_t whence)
{
    int err = 0;
