    ...
    if ((0 <= x) && (x < img->w) && (0 <= y) && (y < img->h)) {
        switch(img->bpp) {
            ...
            case IMAGE_BPP_RGB888: {
                return IMAGE_GET_RGB888_PIXEL(img, x, y);
            }
            default: {
                return -1;
            }
    ...
    switch (dst->bpp) {
        case IMAGE_BPP_BINARY: {
            switch (src->bpp) {
                ...
                case IMAGE_BPP_RGB888: {
                    return COLOR_RGB888_TO_BINARY(pixel);
                }
                default: {
                    return 0;
                }
        ...
        case IMAGE_BPP_GRAYSCALE: {
            switch (src->bpp) {
                ...
                case IMAGE_BPP_RGB888: {
                    return COLOR_RGB888_TO_GRAYSCALE(pixel);
                }
                default: {
                    return 0;
                }
        ...
        case IMAGE_BPP_RGB888: {
            switch (src->bpp) {
                case IMAGE_BPP_BINARY: {
                    return COLOR_BINARY_TO_RGB888(pixel);
                }
                case IMAGE_BPP_GRAYSCALE: {
                    return COLOR_GRAYSCALE_TO_RGB888(pixel);
                }
                case IMAGE_BPP_RGB565: {
                    return COLOR_RGB565_TO_RGB888(pixel);
                }
                case IMAGE_BPP_RGB888: {
                    return pixel;
                }
                        ...
                        int c, bool invert, bool clear_background, image_t *mask)
{
    ...
    if ((0 <= x) && (x < img->w) && (0 <= y) && (y < img->h))
    {
        ...
        case IMAGE_BPP_RGB888:
        {
            for (int y = 0, yy = out.h; y < yy; y++)
            {
                uint32_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                uint32_t *out_row_ptr = IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(&out, y);
                for (int x = 0, xx = out.w; x < xx; x++)
                {
                    if (IMAGE_GET_BINARY_PIXEL_FAST(out_row_ptr, x) ^ invert)
