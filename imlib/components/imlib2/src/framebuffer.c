...
{
    framebuffer = (framebuffer_t *)malloc(OMV_FB_SIZE + OMV_FB_ALLOC_SIZE);
    jpeg_framebuffer  = (jpegbuffer_t *)malloc(OMV_JPEG_BUF_SIZE);
    ...
    if (framebuffer->streaming_enabled && jpeg_framebuffer->enabled) {
        if (src->bpp > 3) {
            ...
            if (does_not_fit) {
                ...
                fb_alloc_mark();
                uint8_t *temp = (uint8_t*)fb_alloc(new_size, FB_ALLOC_NO_HINT);
