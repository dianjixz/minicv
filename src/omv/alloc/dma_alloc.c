...
{
    DBGLOG_ERROR("DMA buffer allocation failed while allocating %u bytes"), (uint) size);
}
...
static void block_chain_from_domain(void *periph, void **start, void **end)
{
    ...
    switch (domain) {
        ...
        #endif
        default:
            DBGLOG_ERROR("DMA buffer allocation failed. No DMA buffer reserved for peripheral 0x%x"), (uint) periph);
            break;
    }
}
