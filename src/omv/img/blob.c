    ...
    for (list_lnk_t *it = iterator_start_from_head(thresholds); it; it = iterator_next(it))
    {
        ...
        case IMAGE_BPP_BINARY:
        {
            for (int y = roi->y, yy = roi->y + roi->h; y < yy; y += y_stride)
            {
                ...
                {
                    if ((!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(row_index, x))) && COLOR_THRESHOLD_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row_ptr, x), &lnk_data, invert))
                    {
                        int old_x = x;
                        int old_y = y;
                        ...
                        for (;;)
                        {
                            ...
                            while ((left > roi->x) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, left - 1))) && COLOR_THRESHOLD_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, left - 1), &lnk_data, invert))
                            {
                                left--;
                            }

                            while ((right < (roi->x + roi->w - 1)) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, right + 1))) && COLOR_THRESHOLD_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, right + 1), &lnk_data, invert))
                            {
                                right++;
                            }
                            ...
                            for (;;)
                            {
                                if (lifo_size(&lifo) < lifo_len)
                                {
                                    ...
                                    if (y > roi->y)
                                    {
                                        ...
                                        for (int i = left; i <= right; i++)
                                        {
                                            if ((!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, i))) && COLOR_THRESHOLD_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, i), &lnk_data, invert))
                                            {
                                                xylf_t context;
                                                context.x = x;
                                                context.y = y;
        ...
        case IMAGE_BPP_GRAYSCALE:
        {
            for (int y = roi->y, yy = roi->y + roi->h; y < yy; y += y_stride)
            {
                ...
                for (int x = roi->x + (y % x_stride), xx = roi->x + roi->w; x < xx; x += x_stride)
                {
                    if ((!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(row_index, x))) && COLOR_THRESHOLD_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row_ptr, x), &lnk_data, invert))
                    {
                        int old_x = x;
                        int old_y = y;
                        ...
                        for (;;)
                        {
                            ...
                            while ((left > roi->x) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, left - 1))) && COLOR_THRESHOLD_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, left - 1), &lnk_data, invert))
                            {
                                left--;
                            }

                            while ((right < (roi->x + roi->w - 1)) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, right + 1))) && COLOR_THRESHOLD_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, right + 1), &lnk_data, invert))
                            {
                                right++;
                            }
                            ...
                            for (;;)
                            {
                                if (lifo_size(&lifo) < lifo_len)
                                {
                                    ...
                                    if (y > roi->y)
                                    {
                                        ...
                                        for (int i = left; i <= right; i++)
                                        {
                                            if ((!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, i))) && COLOR_THRESHOLD_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, i), &lnk_data, invert))
                                            {
                                                xylf_t context;
                                                context.x = x;
                                                context.y = y;
                ...
                for (int x = roi->x + (y % x_stride), xx = roi->x + roi->w; x < xx; x += x_stride)
                {
                        ...
                        COLOR_THRESHOLD_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row_ptr, x), &lnk_data, invert))
                    {
                        ...
                        for (;;)
                        {
                            ...
                            while ((left > roi->x) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, left - 1))) && COLOR_THRESHOLD_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, left - 1), &lnk_data, invert))
                            {
                                left--;
                            }

                            while ((right < (roi->x + roi->w - 1)) && (!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, right + 1))) && COLOR_THRESHOLD_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, right + 1), &lnk_data, invert))
                            {
                                right++;
                            }
                            ...
                            for (;;)
                            {
                                if (lifo_size(&lifo) < lifo_len)
                                {
                                    ...
                                    if (y > roi->y)
                                    {
                                        ...
                                        for (int i = left; i <= right; i++)
                                        {
                                            if ((!bitmap_bit_get(&bitmap, BITMAP_COMPUTE_INDEX(index, i))) && COLOR_THRESHOLD_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, i), &lnk_data, invert))
                                            {
                                                xylf_t context;
                                                context.x = x;
                                                context.y = y;
                            ...
                            flood_fill_call_back_t cb, void *data)
{
    ...
    case IMAGE_BPP_BINARY:
    {
        for (int seed_pixel = IMAGE_GET_BINARY_PIXEL(img, x, y);;)
        {
            ...
            while ((left > 0) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, left - 1)) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, left - 1), seed_pixel, seed_threshold) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, left - 1), IMAGE_GET_BINARY_PIXEL_FAST(row, left), floating_threshold))
            {
                left--;
            }

            while ((right < (img->w - 1)) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, right + 1)) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, right + 1), seed_pixel, seed_threshold) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, right + 1), IMAGE_GET_BINARY_PIXEL_FAST(row, right), floating_threshold))
            {
                right++;
            }
            ...
            for (;;)
            {
                if (lifo_size(&lifo) < lifo_len)
                {
                    ...
                    if (y > 0)
                    {
                        ...
                        for (int i = left; i <= right; i++)
                        {
                            if ((!IMAGE_GET_BINARY_PIXEL_FAST(out_row, i)) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, i), seed_pixel, seed_threshold) && COLOR_BOUND_BINARY(IMAGE_GET_BINARY_PIXEL_FAST(row, i), IMAGE_GET_BINARY_PIXEL_FAST(old_row, i), floating_threshold))
                            {
                                xylf_t context;
                                context.x = x;
                                context.y = y;
    ...
    case IMAGE_BPP_GRAYSCALE:
    {
        for (int seed_pixel = IMAGE_GET_GRAYSCALE_PIXEL(img, x, y);;)
        {
            ...
            while ((left > 0) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, left - 1)) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, left - 1), seed_pixel, seed_threshold) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, left - 1), IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, left), floating_threshold))
            {
                left--;
            }

            while ((right < (img->w - 1)) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, right + 1)) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, right + 1), seed_pixel, seed_threshold) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, right + 1), IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, right), floating_threshold))
            {
                right++;
            }
            ...
            for (;;)
            {
                if (lifo_size(&lifo) < lifo_len)
                {
                    ...
                    if (y > 0)
                    {
                        ...
                        for (int i = left; i <= right; i++)
                        {
                            if ((!IMAGE_GET_BINARY_PIXEL_FAST(out_row, i)) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, i), seed_pixel, seed_threshold) && COLOR_BOUND_GRAYSCALE(IMAGE_GET_GRAYSCALE_PIXEL_FAST(row, i), IMAGE_GET_GRAYSCALE_PIXEL_FAST(old_row, i), floating_threshold))
                            {
                                xylf_t context;
                                context.x = x;
                                context.y = y;
    ...
    case IMAGE_BPP_RGB565:
    {
        for (int seed_pixel = IMAGE_GET_RGB565_PIXEL(img, x, y);;)
        {
            ...
            while ((left > 0) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, left - 1)) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, left - 1), seed_pixel, seed_threshold) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, left - 1), IMAGE_GET_RGB565_PIXEL_FAST(row, left), floating_threshold))
            {
                left--;
            }

            while ((right < (img->w - 1)) && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, right + 1)) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, right + 1), seed_pixel, seed_threshold) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, right + 1), IMAGE_GET_RGB565_PIXEL_FAST(row, right), floating_threshold))
            {
                right++;
            }
            ...
            for (;;)
            {
                if (lifo_size(&lifo) < lifo_len)
                {
                    ...
                    if (y > 0)
                    {
                        ...
                        for (int i = left; i <= right; i++)
                        {
                            if ((!IMAGE_GET_BINARY_PIXEL_FAST(out_row, i)) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, i), seed_pixel, seed_threshold) && COLOR_BOUND_RGB565(IMAGE_GET_RGB565_PIXEL_FAST(row, i), IMAGE_GET_RGB565_PIXEL_FAST(old_row, i), floating_threshold))
                            {
                                xylf_t context;
                                context.x = x;
                                context.y = y;
